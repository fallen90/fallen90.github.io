...
Represents a [RetrieveLoyaltyPromotionPromotions](../api/loyalty.md#retrieve-loyalty-promotion) response.
...
| `loyaltyPromotion` | [`LoyaltyPromotion \| undefined`](../models/loyalty-promotion.md) | Optional | Represents a promotion for a [loyalty program](../models/loyalty-program.md). Loyalty promotions enable buyers<br/>to earn extra points on top of those earned from the base program.<br/><br/>A loyalty program can have a maximum of 10 loyalty promotions with an `ACTIVE` or `SCHEDULED` status. |
