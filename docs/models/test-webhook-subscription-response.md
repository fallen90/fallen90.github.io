...
a request to the [TestWebhookSubscription](../api/webhook-subscriptions.md#test-webhook-subscription) endpoint.

Note: If there are errors processing the request, the [SubscriptionTestResult](../models/subscription-test-result.md) field is not
present.
...
| `subscriptionTestResult` | [`SubscriptionTestResult \| undefined`](../models/subscription-test-result.md) | Optional | Represents the details of a webhook subscription, including notification URL,<br/>event types, and signature key. |
